# create-antd-admin（packages/create）设计规格
...
在 monorepo 中新增 **`packages/create`**，以 npm 包 **`create-antd-admin`** 发布，提供与 [create-turbo](https://github.com/vercel/turborepo/tree/main/packages/create-turbo) 相近体验的命令行工具：从 **GitHub 上的 `apps/<example>`** 拉取模板，在用户本机生成 **独立可运行** 的前端应用，并支持 **交互式选择模板**、**包管理器选择与安装**、**项目名称 transforms**。

## 2. 目标与非目标

### 2.1 目标（v1）

- 发布 **`create-antd-admin`**，`pnpm dlx create-antd-admin@latest` 可用；`bin` 指向构建产物 `dist/cli.js`；npm **`files`** 以 **`dist`**（及必要的内嵌清单）为主，对齐 create-turbo 的发布边界。
...
- **CLI**：
  - 支持 `pnpm dlx create-antd-admin@latest --example basic`（及清单中的其它示例名）。
...
- 包名：**`create-antd-admin`**。
- `bin`：**`create-antd-admin`** → `dist/cli.js`。
...
| 话题 | 结论 |
|------|------|
| 包位置 | `packages/create`，发布名 `create-antd-admin`。 |
